...
public class FollowTarget : MonoBehaviour {
	...
	void Update ()
    {
        ...
        {
            //Movimentar
            transform.Translate((target.transform.position - transform.position).normalized * velocidadeMovimento * Time.deltaTime);
        }
	}
    ...
    private void ProcurarTarget()
    {
        ...
        if (targets.Length > 0)
        {
            ...
            foreach (GameObject checarTarget in targets)
            {
                ...
                if (possivelTarget == null || checarDistancia < Vector3.Distance(possivelTarget.transform.position, transform.position))
                {
